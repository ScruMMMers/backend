...
import com.quqee.backend.internship_hits.model.rest.FileUploadResponseView
...
class FileController(
    ...
    override fun filesPost(file: MultipartFile?): ResponseEntity<FileUploadResponseView> {
        if (file != null) {
            val result = fileService.uploadFile(file)
            return ResponseEntity.ok(FileUploadResponseView(result.fileId))
        } else {
